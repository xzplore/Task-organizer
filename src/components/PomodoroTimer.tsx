    ...
    <section className="bg-zinc-200 dark:bg-zinc-800 p-6 rounded-lg shadow-lg flex flex-col items-center">
      <div className="flex gap-2 mb-6">
        {(['work', 'shortBreak', 'longBreak'] as Mode[]).map(m => (
            <button key={m} onClick={() => selectMode(m)} className={`px-4 py-2 text-sm font-bold rounded-full transition-colors ${mode === m ? 'bg-cyan-600 dark:bg-cyan-500 text-white dark:text-zinc-900' : 'bg-zinc-300 dark:bg-zinc-700 hover:bg-zinc-400 dark:hover:bg-zinc-600'}`}>
                {modeText[m]}
            </button>
        ))}
      </div>
      <div className="w-64 h-64 relative mb-6">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={[{value: 100}]} dataKey="value" stroke={theme === 'dark' ? '#3f3f46' : '#d4d4d8'} fill="transparent" />
            <Pie
              data={data}
              cx="50%"
              cy="50%"
      ...
      <div className="flex gap-4">
        <button
          onClick={toggleTimer}
          className="bg-cyan-600 text-white dark:bg-cyan-500 dark:text-zinc-900 font-bold px-10 py-3 rounded-lg hover:bg-cyan-500 dark:hover:bg-cyan-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-200 dark:focus:ring-offset-zinc-800 focus:ring-cyan-500 transition-colors text-2xl"
        >
          {isActive ? 'إيقاف مؤقت' : 'ابدأ'}
        </button>
        <button
            onClick={resetTimer}
            className="p-3 text-zinc-500 dark:text-zinc-400 hover:bg-zinc-300 dark:hover:bg-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 transition-colors"
