...
const NavigationMenu: React.FC<NavigationMenuProps> = ({ isOpen, onClose, onNavigate, currentView }) => {
  ...
  const inactiveLinkStyles = "text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700";
  ...
  return (
    <div className="fixed inset-0 z-40" role="dialog" aria-modal="true">
      ...
      {/* Menu Panel */}
      <div className="absolute top-0 right-0 h-full w-72 bg-slate-100 dark:bg-slate-800 shadow-2xl p-6 flex flex-col">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-2xl font-bold text-cyan-600 dark:text-cyan-400">القائمة</h2>
          <button onClick={onClose} className="p-2 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700" aria-label="Close menu">
