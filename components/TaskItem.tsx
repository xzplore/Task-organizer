...
const TaskItem: React.FC<TaskItemProps> = ({ task, onToggle, onDelete, isAdmin = false }) => {
  ...
  const lockedBg = isPastDueAndIncomplete ? 'bg-red-200/30 dark:bg-red-900/30 hover:bg-red-200/30 dark:hover:bg-red-900/30' : 'bg-slate-200 dark:bg-slate-800 hover:bg-slate-300/50 dark:hover:bg-slate-700/50';
  ...
  return (
    <li className={`flex items-center p-3 rounded-lg shadow-md transition-all duration-300 border-l-4 ${priorityStyles[task.priority]} ${lockedBg}`}>
      ...
      >
         <div className={`mt-1 w-6 h-6 flex-shrink-0 rounded-full border-2 ${task.completed ? 'border-green-500 bg-green-500' : 'border-slate-400 dark:border-slate-500'} flex items-center justify-center transition-all duration-300 ${isPastDueAndIncomplete ? 'opacity-40 cursor-not-allowed' : ''}`}>
          {task.completed && (
        ...
        <div className="mr-4 flex flex-col">
            ...
            <div className="flex items-center flex-wrap gap-3 mt-1 text-xs">
                ...
                {task.dueDate && (
                     <span className={`font-semibold ${isPastDueAndIncomplete ? 'text-red-500' : 'text-slate-500 dark:text-slate-400'}`}>
      ...
      <button
        ...
        className="text-slate-500 dark:text-slate-400 hover:text-red-500 dark:hover:text-red-400 transition-colors self-start flex-shrink-0 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:text-slate-500 dark:disabled:hover:text-slate-400"
